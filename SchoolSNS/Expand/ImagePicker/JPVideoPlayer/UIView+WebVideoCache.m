...
HSGHVideoDownloadView *downloadView;
...
- (void)jp_playVideoWithURL:(NSURL *)url{
    [self jp_playVideoWithURL:url options:JPVideoPlayerContinueInBackground | JPVideoPlayerLayerVideoGravityResizeAspect
...
- (void)jp_playVideoHiddenStatusViewWithURL:(NSURL *)url{
    [self jp_playVideoWithURL:url options:JPVideoPlayerContinueInBackground  | JPVideoPlayerLayerVideoGravityResizeAspect progress:nil completed:nil];
}

- (void)jp_playVideoMutedDisplayStatusViewWithURL:(NSURL *)url{
    [self jp_playVideoWithURL:url options:JPVideoPlayerContinueInBackground | JPVideoPlayerShowProgressView  | JPVideoPlayerLayerVideoGravityResizeAspect | JPVideoPlayerMutedPlay progress:nil completed:nil];
}
...
- (BOOL)videoPlayerManager:(JPVideoPlayerManager *)videoPlayerManager downloadingProgressDidChanged:(CGFloat)downloadingProgress{
    NSLog(@"downloadingProgress:%f",downloadingProgress);
    dispatch_async(dispatch_get_main_queue(), ^{
        if (downloadingProgress >= 1) {
            ...
            if (downloadView == nil) {
                downloadView = [[HSGHVideoDownloadView alloc] initWithFrame:CGRectMake(self.width/2-30, self.height/2-30, 60, 60)];
            ...
            if (![self.subviews containsObject:downloadView]) {
                [downloadView setProgress:downloadingProgress];
                [self addSubview:downloadView];
                [downloadView mas_remakeConstraints:^(MASConstraintMaker *make) {
                    make.center.equalTo(self);
                    make.width.equalTo(@60);
            ...
            if (self.playingStatus == JPVideoPlayerPlayingStatusPlaying) {
                [self jp_pause];
            }
            [downloadView setProgress:downloadingProgress];
        }
