...
@interface HSGHHomeFirstView () <UITableViewDelegate, UITableViewDataSource,
...
@end
@implementation HSGHHomeFirstView
...
- (UITableViewCell*)tableView:(UITableView*)tableView cellForRowAtIndexPath:(NSIndexPath*)indexPath
{
    ...
    cell.forwardBtnClickBlock = ^{
        [self forwardWithQqianVO:tmpModelF.model succBlock:^{
            //成功后刷新当前cell
            Toast *toast = [[Toast alloc] initWithText:@"转发成功" delay:0 duration:1.f];
            [toast show];
            [self forwardSuccRefreshCurrCell:indexPath];
        }];
    };
    ...
    cell.upBtnClickBlock = ^ (CGRect frame){
        ...
        [HSGHNetworkSession postReq:HSGHHomeQQiansUPURL
                       appendParams:@{@"qqianId" : tmpModelF.model.qqianId }
    ...
    cell.headShareBtnClickBlock = ^{
        ...
        view.block = ^(NSInteger type){
            ...
            HSLog(@"---homeFirstView---share---删除成功后---qqianId=%@",tmpModelF.model.qqianId);
            [weakSelf deleteItemByQQid:tmpModelF.model.qqianId andType:type];//type=1000 删原创，1001删转发
        };
        [[AppDelegate instanceApplication].window.rootViewController.view addSubview:view];
        [view loadDataWithModel:tmpModelF WithCellView:weakCell];
    };
