    ...
    if (self.photoManager.singleSelecteClip) {
        ...
        if (self.model.type == HXPhotoModelMediaTypeCameraPhoto) {
            ...
            self.requestID = [HXPhotoTools getPhotoForPHAsset:self.model.asset size:PHImageManagerMaximumSize completion:^(UIImage *image, NSDictionary *info) {
                ...
                NSData * imageData = UIImageJPEGRepresentation(weakSelf.imageView.image,1);
                NSLog(@"---block---daxiao:%ld k",[imageData length]/1024);
...
- (void)clipImage {
    ...
    UIImage *image = [self imageFromView:self.scrollView atFrame:CGRectMake(10, (height / 2 - diameter / 2), diameter, diameter)];
    
    
    NSData * imageDatad = UIImageJPEGRepresentation(self.imageView.image,1);
    NSLog(@"---aaaaaaaa---daxiao:%ld k",[imageDatad length]/1024);
...
//    CGRect rect = CGRectMake(0, 100, 375, 375);
...
//    
    ...
    NSLog(@"---hou---daxiao:%ld k",[imageData length]/1024);
...
- (UIImage *)imageFromView: (UIView *) theView   atFrame:(CGRect)r {
    ...
    NSLog(@"---theView---daxiao:%ld k",[imageData length]/1024);
