...
- (void)viewDidLoad {
    ...
    if ([PHPhotoLibrary authorizationStatus] != PHAuthorizationStatusAuthorized) {
        ...
        self.timer = [NSTimer scheduledTimerWithTimeInterval:0.5 target:self selector:@selector(observeAuthrizationStatusChange:) userInfo:nil repeats:YES];
...
- (UINavigationBar *)navBar {
    if (!_navBar) {
        ...
        _navBar = [[UINavigationBar alloc] initWithFrame:CGRectMake(0, y, self.view.hx_w, 64)];
...
- (UINavigationItem *)navItem {
    if (!_navItem) {
        ...
        if (!_hiddenLeftBackButton) {//第一次注册，强制发一次新鲜事，不提供返回按钮
            _navItem.leftBarButtonItem = [[UIBarButtonItem alloc] initWithCustomView:cancelBtn];
        }
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (model.type == HXPhotoModelMediaTypeCamera) {//第一个
        //if (self.manager.cameraType == HXPhotoManagerCameraTypeFullScreen) {
            
            if (self.manager.type == HXPhotoManagerSelectedTypeVideo) {
                if (self.manager.cameraType == HXPhotoManagerCameraTypeHalfScreen) {
                    HXCameraViewController *vc = [[HXCameraViewController alloc] init];
                    vc.delegate = self;
                    vc.type = HXCameraTypeVideo;//摄像
                    vc.photoManager = self.manager;
...
//                [self.navigationController pushViewController:vc animated:YES];
        ...
        [manager requestImageDataForAsset:model.asset options:PHImageContentModeDefault resultHandler:^(NSData * _Nullable imageData, NSString * _Nullable dataUTI, UIImageOrientation orientation, NSDictionary * _Nullable info) {
            UIImage *resultImage = [UIImage imageWithData:imageData];
            NSLog(@"裁剪之前width:%f,height:%f",resultImage.size.width,resultImage.size.height);
            HSGHImageDiamondViewController *diamondVC = [[HSGHImageDiamondViewController alloc] initWithDiamondImage:resultImage];
            diamondVC.nextStepClick = ^(UIImage *image) {
                NSLog(@"裁剪之后width:%f,height:%f",image.size.width,image.size.height);
        ...
        [manager requestAVAssetForVideo:model.asset options:options resultHandler:^(AVAsset * _Nullable asset, AVAudioMix * _Nullable audioMix, NSDictionary * _Nullable info) {
            ...
            CMTime aTime = [asset duration];
            Float64 duration = CMTimeGetSeconds(aTime);
            HSLog(@"---选择了一个视频------duration=%f",duration);
            if (duration < kMaxVideoTime+1) {//包含10.5
                HSLog(@"选择了一个视频---直接压缩后发布");
                [[AppDelegate instanceApplication] indicatorShow];
                [weakSelf saveVideo_Me:asset andCompleteBlock:^(BOOL success, NSString* outputPath) {
                    dispatch_async(dispatch_get_main_queue(), ^{
                        ...
                        if (success) {
                            ...
                            //HSLog(@"选择了一个视频---直接压缩后发布---videoImage.size=%@",NSStringFromCGSize(videoImage.size));
...
//            if (duration <= kMaxVideoTime) {//小于最大时长
//                dispatch_async(dispatch_get_main_queue(), ^{
//                    HSGHPublishMsgVC *vc = [HSGHPublishMsgVC new];
//                    vc.publishType = @1;
//                    vc.image = cell.imageView.image;
//                    AVURLAsset *urlAsset = (AVURLAsset *)asset;
//                    vc.localVideoPath = [urlAsset.URL absoluteString];
//                    [self.navigationController pushViewController:vc animated:YES];
//                });
//                
//                
...
- (void)saveVideo_Me:(AVAsset *)asset andCompleteBlock:(void (^)(BOOL success, NSString* urlPath))handler {
    //[self.player pause];
    ...
    [exportSession exportAsynchronouslyWithCompletionHandler:^{
        switch ([exportSession status]) {
            case AVAssetExportSessionStatusFailed:{
                ...
                HSLog(@"压缩完毕,压缩后大小 %f MB,url:%@",[self fileSize:exportSession.outputURL],exportSession.outputURL);
