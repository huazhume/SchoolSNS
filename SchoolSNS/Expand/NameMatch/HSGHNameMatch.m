...
+ (BOOL)isMatchName:(NSString*)name {
    ...
    NSArray* nameArray = @[@"赵", @"钱", @"孙", @"李", @"周", @"吴", @"郑", @"王", @"冯", @"陈",@"褚",  @"卫",
    ...
    for (NSString* nameCell in nameArray) {
        if ([nameCell isEqualToString:name]) {
            return true;
        }
    }
...
+ (NSString *)pinyinOfString:(NSString*)nameStr {
    if ([nameStr isEqualToString:@"曾"]) {
        return @"zeng";
    }

    NSMutableString * name = [[NSMutableString alloc] initWithString:nameStr ];
...
+ (NSString *)pinyinMultiOfString:(NSString*)nameStr {
    ...
    for(int i =0; i < nameStr.length ; i++) {
        name = [name stringByAppendingString:[HSGHNameMatch pinyinOfString:[nameStr substringWithRange:NSMakeRange(i, 1)]]];
    }
