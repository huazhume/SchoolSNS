...
- (void)playViewLongPgrEvent:(UILongPressGestureRecognizer *)longPgr {
    ...
    if (longPgr.state == UIGestureRecognizerStateCancelled ||
        ...
        [self.playView clean];
        if (self.videoTime < kMinVideoTime) {
            self.changeCameraBtn.hidden = NO;
            if (self.flashBtn.enabled) {
                self.flashBtn.hidden = NO;
            }
            ...
            [self.view showImageHUDText:[NSString stringWithFormat:@"录制时间少于%d秒",kMinVideoTime]];
            self.timeLb.text = self.titleStr;
        }else {
            [self hideClick];
        }
...
- (void)recordingClick:(NSTimer *)timer {
    ...
    self.playView.progress = (CGFloat)self.videoTime*1.0 / kMaxVideoTime;
    if (self.videoTime == kMaxVideoTime) {
        [timer invalidate];
        self.timer = nil;
    }
}
...
//        }
        ...
        self.timer = nil;
        if (self.videoTime < kMinVideoTime) {
            //[NSBundle hx_localizedStringForKey:@"录制时间少于3秒"]
            [self.view showImageHUDText:[NSString stringWithFormat:@"录制时间少于%d秒",kMinVideoTime]];
            return;
        }
        [self.playerLayer.player pause];
