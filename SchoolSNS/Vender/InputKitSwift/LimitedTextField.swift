...
open class LimitedTextField: UITextField {
    ...
    @IBInspectable open var limitedRegEx: String? {
        didSet {
            if let regEx = limitedRegEx, (regEx as NSString).length > 0 {
                limitedRegExs = [regEx]
            }
        }
    }
...
//    }
    ...
    @objc fileprivate func textFieldTextDidChange(notification: Notification) {
        ...
        if position == nil {
            var flag = false
            if let count = (currentText as NSString?)?.length, count > maxLength {
                textField?.text = String((currentText!.characters).dropLast(count - maxLength))
                flag = true
            }
            ...
            if self.isTextSelecting && !isMatch {
                ...
                if let hisText = self.historyText,
                    ...
                    (hisText as NSString).length <= (curText as NSString).length {
...
fileprivate class LimitedTextFieldDelegate: LimitedDelegate, UITextFieldDelegate {
    ...
    func textField(_ textField: UITextField, shouldChangeCharactersIn range: NSRange, replacementString string: String) -> Bool {
        ...
        if textField.isKind(of: LimitedTextField.self) {
            ...
            matchStr?.insert(contentsOf: string.characters, at: index!)
            ...
            case .custom:
                if limitedTextField.isTextSelecting {
                    ...
                    if limitedTextField.isFirstNameBig, matchResult, let matchStr = matchStr, (matchStr as NSString).length == 1 {
