...
- (void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex {
    if (buttonIndex == 0) {
        if(![UIImagePickerController isSourceTypeAvailable:UIImagePickerControllerSourceTypeCamera]) {
            [[self viewController:self].view showImageHUDText:@"无法使用相机!"];
            return;
        }
    ...
    if (self.manager.saveSystemAblum) {
        if ([PHPhotoLibrary authorizationStatus] != PHAuthorizationStatusAuthorized) {
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.4 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                [[self viewController:self].view showImageHUDText:@"保存失败，无法访问照片\n请前往设置中允许访问照片"];
            });
            return;
        }
        ...
        if (model.type == HXPhotoModelMediaTypeCameraPhoto) {
            [HXPhotoTools saveImageToAlbum:model.previewPhoto completion:^{
                ...
                dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.4 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                    [[weakSelf viewController:weakSelf].view showImageHUDText:@"照片保存失败!"];
                });
            }];
        }else {
            [HXPhotoTools saveVideoToAlbum:model.videoURL completion:^{
                ...
                weakSelf.manager.cameraPhoto = YES;
                dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.4 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                    [[weakSelf viewController:weakSelf].view showImageHUDText:@"视频保存失败!"];
                });
            }];
        }
        return;
