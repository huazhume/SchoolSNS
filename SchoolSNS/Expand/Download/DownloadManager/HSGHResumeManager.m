...
@interface HSGHResumeManager () <NSURLSessionDelegate, NSURLSessionTaskDelegate>

@property(nonatomic, strong)
    NSURLSession *session; //注意一个session只能有一个请求任务
...
@implementation HSGHResumeManager
 ...
 *  @param progress 进度
 */
- (void)setProgressBlockWithProgress:
    (void (^)(long long totalReceivedContentLength,
  ...
  self.progressBlock = ^{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
      ...
      progress(weakSelf.totalReceivedContentLength,
               weakSelf.totalContentLength);
    });
  };
}
...
- (void)start {
  ...
  long long downloadedBytes = self.totalReceivedContentLength =
      [self fileSizeForPath:self.targetPath];
  if (downloadedBytes > 0) {
    ...
    int fileDescriptor =
        open([self.targetPath UTF8String], O_CREAT | O_EXCL | O_RDWR, 0666);
    if (fileDescriptor > 0) {
...
- (void)URLSession:(NSURLSession *)session
  ...
  HSLog(@"didBecomeInvalidWithError");
}
...
- (void)URLSession:(NSURLSession *)session
  ...
  if (response.statusCode == 200) {
    ...
    NSString *contentRange =
        [response.allHeaderFields valueForKey:@"Content-Range"];
    if ([contentRange hasPrefix:@"bytes"]) {
      NSArray *bytes = [contentRange
    ...
    if ([contentRange hasPrefix:@"bytes"]) {
      ...
      if ([bytes count] == 3) {
        ...
        if (self.totalReceivedContentLength == self.totalContentLength) {
          ...
          self.error =
              [[NSError alloc] initWithDomain:[self.url absoluteString]
                                         code:416
