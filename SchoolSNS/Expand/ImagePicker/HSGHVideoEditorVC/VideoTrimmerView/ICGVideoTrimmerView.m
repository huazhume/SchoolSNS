...
@property (nonatomic) CGFloat maxSpaceWidth;//对应的左右最大间距
...
- (instancetype)initWithFrame:(CGRect)frame asset:(AVAsset *)asset
{
    ...
    if (self) {
        _asset = asset;
        _startTime = 0;
        _endTime = kMaxVideoTime;
        [self resetSubviews];
        
        _maxSpaceWidth = (HSGH_SCREEN_WIDTH-2*10)*kMaxVideoTime*1.0/CMTimeGetSeconds(asset.duration);
    }
    return self;
}
...
- (void)resetSubviews
{
    ...
    CGFloat rightViewFrameX = (HSGH_SCREEN_WIDTH-2*self.thumbWidth)*kMaxVideoTime*1.0/self.maxLength+self.thumbWidth;
    //HSLog(@"---rightOverlayView.frame=%@",NSStringFromCGRect(CGRectMake(rightViewFrameX, 0, self.overlayWidth, CGRectGetHeight(self.frameView.frame))));
    ...
    switch (gesture.state) {
        ...
        case UIGestureRecognizerStateChanged:
        {
            ...
            if (_isDraggingRightOverlayView){
                ...
                CGFloat tmpRX = self.rightOverlayView.frame.origin.x;
                CGFloat tmpLMX = CGRectGetMaxX(self.leftOverlayView.frame);
                if (tmpRX-tmpLMX>_maxSpaceWidth) {
                    HSLog(@"---要处理左边的leftOverlayView");
                    
                    CGFloat deltaX = tmpRX-tmpLMX-_maxSpaceWidth;
                ...
                CGFloat tmpLMX = CGRectGetMaxX(self.leftOverlayView.frame);
                if (tmpRX-tmpLMX>_maxSpaceWidth) {
                    HSLog(@"---要处理右边的rightOverlayView");
                    
                    CGFloat deltaX = tmpRX-tmpLMX-_maxSpaceWidth;
                    CGPoint center = self.rightOverlayView.center;
                    center.x -= deltaX;
                    CGFloat newRightViewMidX = center.x;
                    self.rightOverlayView.center = CGPointMake(newRightViewMidX, self.leftOverlayView.center.y);
                }
...
{
    HSLog(@"leftOverlayView:%f , rightOverlayView:%f contentOffset.x:%@", CGRectGetMaxX(self.leftOverlayView.frame) , CGRectGetMaxX(self.rightOverlayView.frame) , @(self.scrollView.contentOffset.x));
