...
- (void)cameraDistrict
{
    ...
    if (IOS11_Later) {
        y = 20;
        UIView *view = [[UIView alloc] initWithFrame:CGRectMake(0, 0, HSGH_SCREEN_WIDTH, 20)];
        [self.view addSubview:view];
        view.backgroundColor = HEXRGBCOLOR(0xfafafa);
    }
    UINavigationBar *navBar = [[UINavigationBar alloc] initWithFrame:CGRectMake(0, y, width, 64)];
...
//    }
    ...
    self.session.sessionPreset = AVCaptureSessionPreset640x480;//AVCaptureSessionPreset1280x720;//分辨率
    //输入输出设备结合
...
//    self.tap = tap;
    ...
    longPGR.minimumPressDuration = 0.7;
...
//    centerIcon.center = CGPointMake(width / 2, CGRectGetMaxY(backView.frame) + 10);
   // [self.view addSubview:centerIcon];
    ...
    self.moveView.center = CGPointMake(width / 2 + self.moveView.frame.size.width / 4, CGRectGetMaxY(backView.frame) + 10 + 10);
    ...
    }else if (self.type == HXCameraTypePhoto) {//拍照
        if ([self.session canAddOutput:self.imageOutput]) {
            [self.session addOutput:self.imageOutput];
        }
    ...
    }else if (self.type == HXCameraTypeVideo) {//摄像
...
- (NSURL *)outPutFileURL
{
    return [NSURL fileURLWithPath:[NSString stringWithFormat:@"%@%@", NSTemporaryDirectory(), [NSString stringWithFormat:@"%@.mp4",[self videoOutFutFileName]]]];
}
...
{
    // 开始录制
    self.timer = [NSTimer scheduledTimerWithTimeInterval:0.2f target:self selector:@selector(recordingClick:) userInfo:nil repeats:YES];
...
- (void)recordingClick:(NSTimer *)timer
{
    ...
    [_changeVideoBtn setTitle:[NSString stringWithFormat:@"%ds",(int)(self.videoTime*0.2)] forState:UIControlStateNormal];
    
    [self.progressView setProgress:self.videoTime*0.2 / kMaxVideoTime animated:YES];
    if (self.videoTime*0.2 == kMaxVideoTime) {
        [timer invalidate];
        [self.videoBtn setImage:[HXPhotoTools hx_imageNamed:self.photoManager.UIManager.recordedBtnNormalImageName] forState:UIControlStateNormal];
        [self.videoOutPut stopRecording];
    }
...
- (void)captureOutput:(AVCaptureFileOutput *)captureOutput didFinishRecordingToOutputFileAtURL:(NSURL *)outputFileURL fromConnections:(NSArray *)connections error:(NSError *)error
{
    ...
    HSLog(@"---录制结束---self.videoURL=%@",self.videoURL);
//    NSData *data = [NSData dataWithContentsOfURL:self.videoURL];
//    HSLog(@"---录制结束---文件大小为：%.2f M",data.length/1024.0/1024.0);
}
...
- (UIView *)moveView
{
    if (!_moveView) {
        ...
        [changePhotoBtn setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
...
- (void)deleteClick
{
    ...
    [_changeVideoBtn setTitle:@"视频" forState:UIControlStateNormal];
}
...
//        NSLog(@"%f k",imageData.length/1024.0);
        
        NSData *pressImageData = UIImageJPEGRepresentation(image, 0.7);
        NSLog(@"%f k",pressImageData.length/1024.0);
        image = [UIImage imageWithData:pressImageData];
        ...
        [self.timer invalidate];
        self.timer = nil;
        if (self.videoTime/5 < kMinVideoTime) {
            [self.view showImageHUDText:[NSString stringWithFormat:@"录制时间小于%d秒,请重新录制",kMinVideoTime]];
            return;
        }
        [self.playerLayer.player pause];
        __weak typeof(self) weakSelf = self;
        ...
        [self clipVideoCompleted:^{
            ...
            NSString *videoTime = [HXPhotoTools getNewTimeFromDurationSecond:weakSelf.videoTime/5];
            ...
            //获取第一张截图
            AVAsset *pressAsset = [AVAsset assetWithURL:model.videoURL];
            AVAssetImageGenerator *assetGen = [[AVAssetImageGenerator alloc] initWithAsset:pressAsset];
            ...
            HSLog(@"---VideoCompleted---去发布界面吧---%@",model.videoURL);
            
            dispatch_async(dispatch_get_main_queue(), ^{
                HSGHPublishMsgVC *vc = [HSGHPublishMsgVC new];
                vc.publishType = @1;
                vc.image = videoImage;
                vc.localVideoPath = [model.videoURL absoluteString];
...
//    AVAsset *audio = [AVAsset assetWithURL:[[NSBundle mainBundle] URLForResource:@"8836.mp3" withExtension:nil]];
    ...
    NSString *lastTmpName = [[self.videoURL pathComponents] lastObject];
...
//                             [NSString stringWithFormat:@"FinalVideo-%d.mp4",arc4random() % 1000]];
    ...
    NSString *myPathDocs =  [documentsDirectory stringByAppendingPathComponent:lastTmpName];
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        while ((filename = [enumerator nextObject])) {
            if ([[filename pathExtension] isEqualToString:@"mp4"] ) {
                [[NSFileManager defaultManager] removeItemAtPath:[documentsDirectory stringByAppendingPathComponent:filename] error:nil];
            }  
        }
    });
    ...
    //AVAssetExportPresetHighestQuality  AVAssetExportPreset960x540
    exporter.outputURL = self.clipVideoURL;
    exporter.outputFileType = AVFileTypeMPEG4;//视频格式  AVFileTypeQuickTimeMovie
    ...
    [exporter exportAsynchronouslyWithCompletionHandler:^{
        switch (exporter.status) {
            ...
            case AVAssetExportSessionStatusCompleted:
            {
                dispatch_async(dispatch_get_main_queue(), ^{
                    if (completed) {
                        HSLog(@"------压缩文件完成clipVideoCompleted---completed");
                        completed();
                    }
                });
            }
            ...
            case AVAssetExportSessionStatusFailed:
            {
                dispatch_async(dispatch_get_main_queue(), ^{
                    if (failed) {
                        HSLog(@"---clipVideoCompleted---failed");
                        failed();
                    }
                });
            }
